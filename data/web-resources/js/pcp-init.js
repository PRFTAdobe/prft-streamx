...
!(function () {
  ...
  const RESULT_COUNT_ELE_ID = 'pcp-result-count';
  const PLACEHOLDER_LCP_IMAGE = `<img id="background-image"
  ...
  width="412" height="400" fetchpriority="high" alt="">`;
  ...
  const mapFacets = (response) => {
    ...
    const buildQuery = (category, facets) => {
     ...
     if (!category.includes(EXCLUDE_LOADMORE_FOR_KEYWORD)) {
         query.params.size = PER_PAGE_MAX
     }
...
`;
  ...
  const handleResponse = (response, availableFacets) => {
    const resultCountContainer = document.getElementById(RESULT_COUNT_ELE_ID);
    resultCountContainer.innerText = `(${response?.hits.total.value})`;
