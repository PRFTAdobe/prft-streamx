...
const mergeCarts = async (guestCartID, loggedinUserCartID) => {
  ...
  const header = {...utilities.HEADERS, 'Authorization': `Bearer ${userSession.getActiveLoginToken()}`} ;
  const shoppingCart = await utilities.fetchRequests(utilities.GRAPHQL_ENDPOINT, 'POST', header, query);
...
const getCustomerCart = async () => {
  ...
  const userCart = await utilities.fetchRequests(utilities.GRAPHQL_ENDPOINT, 'POST', {...utilities.HEADERS, 'Authorization': `Bearer ${userSession.getActiveLoginToken()}`}, query);
